...
/*
static void authenticationDialogResponseCallback(Fl_Widget* authDialog, int responseID, WebKitAuthenticationWidget* authWidget)
{
...
Fl_Widget* createAuthenticationDialog(Fl_Window* parentWindow, const WebCore::AuthenticationChallenge& challenge)
{
/*
	FLTKDialog* dialog = GTK_DIALOG(gtk_dialog_new());
	gtk_dialog_add_buttons(dialog, GTK_STOCK_CANCEL, GTK_RESPONSE_CANCEL, GTK_STOCK_OK, GTK_RESPONSE_OK, NULL);
	gtk_dialog_set_default_response(dialog, GTK_RESPONSE_OK);

	Fl_Widget* contentArea = gtk_dialog_get_content_area(dialog);
	gtk_container_set_border_width(GTK_CONTAINER(dialog), 5);

	Fl_Widget* authWidget = webkitAuthenticationWidgetNew(challenge, mode);
	gtk_box_pack_start(GTK_BOX(contentArea), authWidget, TRUE, TRUE, 0);
	gtk_widget_show(authWidget);

	Fl_Widget* actionArea = gtk_dialog_get_action_area(dialog);
	gtk_container_set_border_width(GTK_CONTAINER(actionArea), 5);
	gtk_box_set_spacing(GTK_BOX(actionArea), 6);

	FLTKWindow* window = GTK_WINDOW(dialog);
