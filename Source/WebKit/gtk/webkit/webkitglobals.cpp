...
WebKitIconDatabase* webkit_get_icon_database()
{
	...
	if (!database)
		database = new WebKitIconDatabase;
...
WebKitFaviconDatabase* webkit_get_favicon_database()
{
	...
	if (!database)
		database = new WebKitFaviconDatabase;
 ...
 * @item: a #FLTKMenuItem of the default context menu
 *
 ...
 * <programlisting>
 * static bool context_menu_cb (WebKitWebView		*webView,
 *									Fl_Widget			*default_menu,
 *									WebKitHitTestResult *hit_test_result,
 *									bool			 triggered_with_keyboard,
 *									gpointer			 user_data)
 * {
 ...
 *		   FLTKMenuItem *item = (GtkMenuItem *)l->data;
 *
 ...
 * </example>
 *
 * Note that you can get the #FLTKAction of any item in the default context menu with
 ...
 */
 /*
WebKitContextMenuAction webkit_context_menu_item_get_action(FLTKMenuItem* item)
{
...
{
	if (!scheme)
		return static_cast<WebKitSecurityPolicy>(0);
...
void webkitInit()
{
	...
	webkit_set_web_database_directory_path("databases");

	WebCore::cacheStorage().setCacheDirectory("cache");

	PageGroup::setShouldTrackVisitedLinks(true);

	webkit_icon_database_set_path(webkit_get_icon_database(), "icondb");
...
{
	return "org.webkit.fltk.WebKitFLTK";
}
