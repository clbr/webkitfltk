...
static PassOwnPtr<WidgetBackingStore> createBackingStore(FLTKWidget* widget, const IntSize& size)
{
...
{
	FLTKWidget* window = gtk_widget_get_toplevel(GTK_WIDGET(m_webView));
...
KeyboardUIMode ChromeClient::keyboardUIMode()
{
	...
	if (DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled())
		tabsToLinks = DumpRenderTreeSupportFLTK::linksIncludedInFocusChain();
...
void ChromeClient::paint(WebCore::Timer<ChromeClient>*)
{
	...
	HashSet<FLTKWidget*> children = m_webView->priv->children;
	HashSet<FLTKWidget*>::const_iterator end = children.end();
	for (HashSet<FLTKWidget*>::const_iterator current = children.begin(); current != end; ++current) {
		if (static_cast<FLTKAllocation*>(g_object_get_data(G_OBJECT(*current), "delayed-allocation"))) {
...
void ChromeClient::contentsSizeChanged(Frame* frame, const IntSize& size) const
{
	...
	FLTKWidget* widget = GTK_WIDGET(m_webView);
	FLTKRequisition requisition;
...
void ChromeClient::scrollbarsModeDidChange() const
{
	...
	FLTKWidget* parent = gtk_widget_get_parent(GTK_WIDGET(m_webView));
	if (!parent || !GTK_IS_SCROLLED_WINDOW(parent))
		return;

	FLTKPolicyType horizontalPolicy = webkit_web_frame_get_horizontal_scrollbar_policy(webFrame);
	FLTKPolicyType verticalPolicy = webkit_web_frame_get_vertical_scrollbar_policy(webFrame);
...
static gboolean onFullscreenFLTKKeyPressEvent(FLTKWidget* widget, GdkEventKey* event, ChromeClient* chromeClient)
{
	switch (event->keyval) {
	case GDK_KEY_Escape:
	case GDK_KEY_f:
...
void ChromeClient::enterFullScreenForElement(WebCore::Element* element)
{
	...
	g_signal_connect(window, "key-press-event", G_CALLBACK(onFullscreenFLTKKeyPressEvent), this);
...
void ChromeClient::exitFullScreenForElement(WebCore::Element*)
{
	...
	g_signal_handlers_disconnect_by_func(window, reinterpret_cast<void*>(onFullscreenFLTKKeyPressEvent), this);
