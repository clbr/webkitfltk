...
#include "FrameNetworkingContextFLTK.h"
#include "FrameTree.h"
#include "FrameView.h"
#include "FLTKPluginWidget.h"
...
/*
	if (DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled()) {
		...
		if (!DumpRenderTreeSupportFLTK::s_authenticationCallback || !DumpRenderTreeSupportFLTK::s_authenticationCallback(username, password, webResource)) {
...
*/
	...
	CredentialStorageMode credentialStorageMode = DisallowPersistentStorage;
/*
	FLTKWidget* toplevel = gtk_widget_get_toplevel(GTK_WIDGET(view));
	FLTKWidget* authDialog = createAuthenticationDialog(widgetIsOnscreenToplevelWindow(toplevel) ? GTK_WINDOW(toplevel) : 0, challenge, credentialStorageMode);
...
{
/*
	WebKitNetworkResponse networkResponse(0);
...
{
	if (!DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled() || !DumpRenderTreeSupportFLTK::s_frameLoadEventCallback)
		return;
	DumpRenderTreeSupportFLTK::s_frameLoadEventCallback(m_frame, DumpRenderTreeSupportFLTK::DidDisplayInsecureContent, 0);
}
...
void FrameLoaderClient::didDetectXSS(const URL&, bool)
{
	if (!DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled() || !DumpRenderTreeSupportFLTK::s_frameLoadEventCallback)
		return;
	DumpRenderTreeSupportFLTK::s_frameLoadEventCallback(m_frame, DumpRenderTreeSupportFLTK::DidDetectXSS, 0);
}
...
void FrameLoaderClient::dispatchDidReceiveServerRedirectForProvisionalLoad()
{
	if (!DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled() || !DumpRenderTreeSupportFLTK::s_frameLoadEventCallback)
		return;
	DumpRenderTreeSupportFLTK::s_frameLoadEventCallback(m_frame, DumpRenderTreeSupportFLTK::DidReceiveServerRedirectForProvisionalLoad, 0);
}
...
void FrameLoaderClient::dispatchDidCancelClientRedirect()
{
	if (!DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled() || !DumpRenderTreeSupportFLTK::s_frameLoadEventCallback)
		return;
	DumpRenderTreeSupportFLTK::s_frameLoadEventCallback(m_frame, DumpRenderTreeSupportFLTK::DidCancelClientRedirect, 0);
}
...
void FrameLoaderClient::dispatchWillPerformClientRedirect(const URL& url, double, double)
{
	if (!DumpRenderTreeSupportFLTK::dumpRenderTreeModeEnabled() || !DumpRenderTreeSupportFLTK::s_frameLoadEventCallback)
		return;
	DumpRenderTreeSupportFLTK::s_frameLoadEventCallback(m_frame, DumpRenderTreeSupportFLTK::WillPerformClientRedirectToURL, url.string().utf8().data());
}
...
{
	return FrameNetworkingContextFLTK::create(core(m_frame));
}
